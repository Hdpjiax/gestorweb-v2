...
import { skeletonRows, softLoading } from "../app-ui.js";
    ...
    <section class="section admin-login-shell">
      <div class="metric stack">
        <div>
          ...
          <p class="muted">Esta sección requiere una licencia admin activa.</p>
        </div>
      </div>
    </section>
  `;
...
function savedConfigBox() {
  ...
  return `
    <div class="callout stack-sm">
      <strong>Configuración admin guardada</strong>
      <span class="muted">${esc(config.supabaseUrl || "Supabase")}</span>
      <span class="small-note">Credenciales cifradas localmente.</span>
      <div class="flex">
        <button class="btn btn-primary" type="button" data-action="admin-resume">usar configuración guardada</button>
    ...
    <section class="section admin-login-shell">
      <form id="adminLoginForm" class="metric admin-login-card stack">
        <div>
          ...
          <p class="muted">La configuración se guarda cifrada en este equipo.</p>
        </div>
        ${ui.adminResumeBusy ? `<div class="callout">${softLoading("conectando con configuración guardada")}</div>` : savedConfigBox()}
        <div class="grid-2">
        ...
        <label class="stack-sm"><span class="label">Service role key</span><input class="input mono" type="password" name="serviceRoleKey" autocomplete="off" required /></label>
        ...
        ${ui.adminError ? `<div class="friendly-error"><span class="offline-badge">revisar conexión</span><h3>No se pudo conectar</h3><p>${esc(ui.adminError)}</p></div>` : ""}
        <button class="btn btn-primary" type="submit">entrar y guardar</button>
      </form>
    </section>
  `;
}
...
function licenseRow(license) {
  ...
  return `
    <div class="admin-license-row">
      <div>
        <strong class="mono">${esc(license.id)}</strong>
        <small>${esc(license.hwid)}</small>
        <small>${esc(securityLine(license))}</small>
      </div>
      ...
      <span class="mono">${license.last_check_at ? esc(new Date(license.last_check_at).toLocaleString()) : "sin validación"}</span>
      <div class="flex right">
        ...
        ${!license.revoked ? `<button class="btn btn-ghost" type="button" data-action="renew-admin-license" data-id="${attr(license.id)}">renovar</button>` : ""}
        ${status === "activa" ? `<button class="btn btn-ghost" type="button" data-action="suspend-admin-license" data-id="${attr(license.id)}">suspender</button>` : ""}
        ${status === "suspendida" ? `<button class="btn btn-ghost" type="button" data-action="reactivate-admin-license" data-id="${attr(license.id)}">reactivar</button>` : ""}
        ${!license.revoked ? `<button class="btn btn-ghost btn-danger" type="button" data-action="revoke-admin-license" data-id="${attr(license.id)}">revocar</button>` : ""}
      </div>
    </div>
  `;
}
...
function historyPanel() {
  ...
  return `
    <div class="metric stack admin-history-panel">
      <div class="between">
        <div><strong>Auditoría / historial</strong><div class="small-note mono">${esc(ui.adminHistoryLicenseId)}</div></div>
      ...
      <div class="admin-history-list">
        ${events.map((event) => `
          <div class="admin-history-item">
            ...
            <div>
              <strong>${esc(event.detail || "sin detalle")}</strong>
              <small>${event.created_at ? esc(new Date(event.created_at).toLocaleString()) : "ahora"}</small>
              <small>${event.actor_license_id ? `Admin: ${esc(event.actor_license_id)}` : ""}</small>
            </div>
          </div>
...
function offlinePanel() {
  if (!ui.adminOffline && !ui.adminError) return "";
  return `
    <div class="friendly-error">
      <span class="offline-badge">sin conexión</span>
      <h3>No se pudo sincronizar con Supabase</h3>
      <p>${esc(ui.adminError || "Revisa tu conexión, la URL del proyecto y las credenciales guardadas.")}</p>
      <div class="flex">
        <button class="btn btn-primary" type="button" data-action="refresh-admin-licenses">reintentar</button>
        <button class="btn btn-ghost" type="button" data-action="admin-resume">reconectar guardado</button>
      </div>
    </div>
  `;
}

function tableContent(filtered, licenses) {
  if (ui.adminLoading && !licenses.length) return skeletonRows(6);
  return filtered.map(licenseRow).join("") || `<div class="network-empty">No hay licencias con esos filtros.</div>`;
}
  ...
  return `
    <section class="section admin-license-shell stack">
      <div class="between">
        <div>
          <div class="label">Panel privado</div>
          <h2>Licencias por días</h2>
          <div class="small-note mono">${esc(ui.adminServerUrl || "Supabase")}</div>
        </div>
        <div class="flex">
          ${ui.adminLoading ? softLoading(ui.adminBusyLabel || "actualizando") : ""}
          <button class="btn btn-ghost" type="button" data-action="refresh-admin-licenses">actualizar</button>
          <button class="btn btn-ghost" type="button" data-action="logout-admin">salir</button>
        </div>
      </div>
      ...
      <div class="grid-4">
        ...
        <div class="metric"><div class="label">Suspendidas</div><strong class="warn">${suspended}</strong></div>
        <div class="metric"><div class="label">Revocadas</div><strong class="fail">${revoked}</strong></div>
        <div class="metric"><div class="label">Alertas</div><strong class="warn">${failures + mismatches}</strong><div class="small-note">fallos + HWID distintos</div></div>
      </div>

      <form id="adminCreateLicenseForm" class="metric stack">
        <div class="between"><strong>Generar nueva licencia</strong><span class="small-note">Solo licencias por tiempo</span></div>
        <div class="admin-license-form days-only">
          <input class="input mono" name="hwid" placeholder="HWID Windows o Android" required />
      ...
      <div class="metric admin-filter-bar days-only">
        <input class="input mono" data-admin-filter="search" value="${attr(filters.search || "")}" placeholder="Buscar por licencia, HWID o IP..." />
      ...
      <div class="admin-license-table">
        <div class="admin-license-head days-only"><span>Licencia / equipo / seguridad</span><span>Plan</span><span>Vencimiento</span><span>Estado</span><span>Última validación</span><span></span></div>
        ${tableContent(filtered, licenses)}
      </div>
    </section>
  `;
}
